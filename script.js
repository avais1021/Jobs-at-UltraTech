        ...
        if (request.status === 200) {
          ...
          `;
            ...
            newVaccFindJobBtn.addEventListener('click', () => {
                const cityLocation = city.value.toUpperCase();
                const functionValue = functionArea.value.toUpperCase();
                console.log('cityLocation', cityLocation);

                if (cityLocation === '' && functionValue === '') {
                    newCrrVacancies__jobs.innerHTML = '';
                    data.Master.slice(0, maxCardCount).forEach((item) => {
                        const newVaccDiv = document.createElement('div');
                        newVaccDiv.classList.add('newCrrVacancies__jobs__card');
                        newVaccDiv.innerHTML = `
                               <div class="newCrrVacancies__jobs__row1">
                    ...
                    const filteredData = data.Master.filter((item) => {
                        const cardLocation = item.location.toUpperCase();
                        const cardFunction = item.fuction.toUpperCase();

                        return cardLocation.includes(cityLocation) && cardFunction.includes(functionValue);
                    });
